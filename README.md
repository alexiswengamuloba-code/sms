...
<head>
    ...
    <style>
        /* TOUT TON STYLE ORIGINAL EST GARDÉ ICI */
        ...
        .p2 { background: #00BCD4; }
        .p3 { background: #8BC34A; }
        .p4 { background: #FF9800; }
        .p5 { background: #673AB7; }
        .p6 { background: #2196f3; }
...
/>
    ...
    <div class="form-content">
        ...
        <button id="submitBtn" class="submit-btn" onclick="envoyerInternet()" disabled>Envoyer par Internet</button>
        <p id="loadingText" style="display:none; text-align:center; color:blue; font-weight:bold;">Envoi en cours...</p>
    </div>
</div>
...
<script>
    ...
    // NOUVELLE FONCTION PAR INTERNET (FINI LES SMS BLOQUÉS)
    async function envoyerInternet() {
        const btn = document.getElementById('submitBtn');
        ...
        const data = {
            ...
            Choix_Quantite: document.getElementById('choix').value,
            Produit: produitSelectionne,
            Duree: dureeSelectionnee,
            Montant: document.getElementById('montant').value + " FC",
            Numero_Client: document.getElementById('numero').value
        };

        try {
            // !!! REMPLACE TON_CODE_ICI PAR TON ID FORMSPREE !!!
            const response = await fetch('https://formspree.io/f/TON_CODE_ICI', {
                method: 'POST',
            ...
            if (response.ok) {
                popup.innerHTML = `<img src="https://cdn-icons-png.flaticon.com/512/148/148767.png">
                                   <p><strong>Félicitations ! Votre commande a été envoyée avec succès à Alexis via Internet.</strong></p>
                                   <button onclick="fermerPopup()" style="background:#28a745; color:white; border:none; padding:10px 20px; border-radius:5px; cursor:pointer;">OK</button>`;
            } else {
                throw new Error();
            }
        } catch (error) {
            loading.style.display = 'none';
            overlay.style.display = 'flex';
            popup.innerHTML = `<p style="color:red;"><strong>Erreur ! Vérifiez votre connexion Internet et réessayez.</strong></p>
                               <button onclick="fermerPopup()" style="background:red; color:white; border:none; padding:10px 20px; border-radius:5px; cursor:pointer;">RETOUR</button>`;
            btn.disabled = false;
        }
    }
